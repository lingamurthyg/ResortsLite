...
public class BookingService {
    ...
    @Value("${aws.secrets.payment.name:resort-payment-api-credentials}")
    ...
    @PostConstruct
    public void init() {
        // FIXED cr-java-0069: Load database credentials from AWS Secrets Manager at startup
    ...
    // FIXED cr-java-0090: Authentication credentials now loaded from AWS Secrets Manager
    // instead of local file storage
    public boolean authenticateUser(String username, String password) {
        try {
            GetSecretValueRequest getSecretValueRequest = GetSecretValueRequest.builder()
                    .secretId("resort-user-credentials-" + username)
                    .build();
            ...
            return storedPasswordHash.equals(sha256Hash(password));
